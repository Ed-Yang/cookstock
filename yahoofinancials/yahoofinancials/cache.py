        ...
        if not _os.path.isdir(cls._cache_dir):
            try:
                ...
                raise _TzCacheException(
                    f"yahoofinancials: Error creating TzCache folder: '{cls._cache_dir}' reason: {err}")
        elif not (_os.access(cls._cache_dir, _os.R_OK) and _os.access(cls._cache_dir, _os.W_OK)):
            raise _TzCacheException(f"yahoofinancials: Cannot read and write in TzCache folder: '{cls._cache_dir}'")
...
class _TzCache:
    ...
    def get_db(self):
        ...
        try:
            ...
            logging.info(f"yahoofinancials: Failed to create TzCache, reason: {err}. "
                         "TzCache will not be used. "
                         "Tip: You can direct cache to use a different location with 'set_tz_cache_location("
                         "mylocation)'")
            self.dummy = True
            return None
        return self.db
    ...
    def store(self, key, value):
        ...
        try:
            ...
            if old_value != value:
                logging.debug(f"yahoofinancials: Value for key {key} changed from {old_value} to {value}.")
                with db.atomic():
        ...
        if not _os.path.isdir(cls._cache_dir):
            try:
                ...
                raise _CookieCacheException(
                    f"yahoofinancials: Error creating CookieCache folder: '{cls._cache_dir}' reason: {err}")
        elif not (_os.access(cls._cache_dir, _os.R_OK) and _os.access(cls._cache_dir, _os.W_OK)):
            raise _CookieCacheException(
                f"yahoofinancials: Cannot read and write in CookieCache folder: '{cls._cache_dir}'")
...
class _CookieCache:
    ...
    def get_db(self):
        ...
        try:
            ...
            logging.info(f"yahoofinancials: Failed to create CookieCache, reason: {err}. "
